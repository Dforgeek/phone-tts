...
class QuantizedDecoderWrapper(nn.Module):
    ...
    def forward(self, x, g=None):
        ...
        o, o_mb = self.dec(x, g=g) # внутри quantized conv1d/convtr1d
        o = self.dequant(o)        # quantized -> float
        o_mb = self.dequant(o_mb)
        return o, o_mb
    


def enable_qat_for_decoder(net_g, backend: str = "fbgemm"):
    print(f"[QAT] Starting QAT fine-tuning of decoder (backend={backend})")

    # Важно: выбрать движок квантования
    torch.backends.quantized.engine = backend

    # 1. Убираем weight_norm только в декодере (ты это уже делал, оставляю для целостности)
    def remove_wn(m):
        try:
    ...
    print("Removing weight norm from decoder...")
    net_g.dec.apply(remove_wn)

    # 2. Вместо get_default_qat_qconfig(backend) берём per-tensor QAT-конфиг
    #    Он не использует per-channel для весов и нормально работает с ConvTranspose1d.
    qat_qconfig = tq_config.default_qat_qconfig  # per-tensor qint8

    net_g.dec = QuantizedDecoderWrapper(net_g.dec)
    net_g.dec.qconfig = qat_qconfig
    tq.prepare_qat(net_g.dec, inplace=True)


    print("[QAT] Enabled QAT for net_g.dec (per-tensor weights, ConvTranspose1d is supported)")
    return net_g
...
def run(rank, n_gpus, hps):
    ...
    if use_qat:
        print("[QAT] Starting QAT fine-tuning of decoder")
        net_g = enable_qat_for_decoder(net_g, backend=qat_backend)
        for param_group in optim_g.param_groups:
            param_group["lr"] = hps.train.learning_rate * 0.1

        # выключаем fp16 на QAT-этапе
    ...
    if getattr(hps.train, "use_qat", False) and rank == 0:
        ...
        qat_backend = getattr(hps.train, "qat_backend", "fbgemm")
        net_g_q = enable_qat_for_decoder(net_g_q, backend=qat_backend)
        ...
        net_g_q.dec = tq.convert(net_g_q.dec, inplace=False)
