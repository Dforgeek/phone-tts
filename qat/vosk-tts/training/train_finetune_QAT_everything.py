...
def prepare_model_for_qat(net_g, backend: str = "fbgemm"):
    """
    ВНИМАНИЕ: теперь это включает QAT для ВЕСЬ net_g, а не только .dec.
    Оставил старое имя, чтобы не менять вызовы ниже.
    """
    print(f"[QAT] Starting QAT fine-tuning of FULL generator (backend={backend})")

    # Движок квантования
    torch.backends.quantized.engine = backend

    # 1. Убираем weight_norm со всех слоёв генератора
    def remove_wn(m):
        try:
    ...
    print("Removing weight norm from FULL net_g...")
    net_g.apply(remove_wn)

    # 2. Пер-тензорный QAT-конфиг (дружит с ConvTranspose1d)
    qat_qconfig = tq_config.default_qat_qconfig  # per-tensor qint8

    # 3. Навешиваем qconfig только на нужные слои
    def set_qconfig(m):
        if isinstance(m, (nn.Conv1d, nn.ConvTranspose1d, nn.Linear)):
            m.qconfig = qat_qconfig

    net_g.apply(set_qconfig)

    # 4. Готовим весь net_g к QAT
    tq.prepare_qat(net_g, inplace=True)

    print("[QAT] Enabled QAT for FULL net_g (all Conv1d/ConvTranspose1d/Linear)")
    return net_g
