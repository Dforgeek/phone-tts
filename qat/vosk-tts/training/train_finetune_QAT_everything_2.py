...
from ptq import prepare_model_for_qat_convs_only
...
def prepare_model_for_qat(net_g, backend: str = "fbgemm"):
    """
    Готовим net_g к QAT, но:
      - снимаем weight_norm со всей модели,
      - QAT-включаем ТОЛЬКО декодер (net_g.dec),
      - внутри dec оборачиваем conv/convT/linear в QuantWrapper
        (см. quantize_toolkit.prepare_model_for_qat_convs_only).

    Важно: оставил имя функции тем же, чтобы не менять вызовы ниже.
    """
    print(f"[QAT] Starting QAT fine-tuning of DECODER only (backend={backend})")

    torch.backends.quantized.engine = backend

    # 1. Убираем weight_norm со ВСЕХ слоёв (и у net_g, и потом у net_g_q)
    def remove_wn(m: nn.Module):
        try:
            nn.utils.remove_weight_norm(m)
        except Exception:
            pass

    print("[QAT] Removing weight norm from FULL net_g...")
    net_g.apply(remove_wn)

    # 2. Готовим модель к QAT только под корнем "dec"
    #    Внутри dec все Conv1d / ConvTranspose1d / Linear будут обернуты QuantWrapper-ами.
    net_g = prepare_model_for_qat_convs_only(
        model=net_g,
        module_roots=None,   # ключевая строка: квантуем ТОЛЬКО декодер
        backend=backend,
    )

    print("[QAT] QAT is ENABLED for net_g.dec (convs/convT/linear via QuantWrapper).")
    return net_g
...
def run(rank, n_gpus, hps):
    ...
    try:
        _, _, _, _ = utils.load_checkpoint("db-finetune/out_ft_100/G_1250.pth", net_g,
                                                   None)
        _, _, _, _ = utils.load_checkpoint("db-finetune/out_ft_100/D_1250.pth", net_d,
                                                   optim_d)
        _, _, _, _ = utils.load_checkpoint("db-finetune/out_ft_100/DUR_1250.pth",
                                                   net_dur_disc, optim_dur_disc)
        if net_wd:
            _, _, _, _ = utils.load_checkpoint("b-finetune/out_ft_100/WD_1250.pth",
                net_wd,
                optim_wd,
            )
    ...
    # ВСТАВКА
    # ---- QAT-режим: дообучаем квантизованный декодер ----
    # Предположим, что в config.json ты добавишь поле:
    # "train": { ..., "use_qat": true, "qat_backend": "fbgemm" }
    use_qat = getattr(hps.train, "use_qat", False)
    qat_backend = getattr(hps.train, "qat_backend", "fbgemm")

    if use_qat:
        print("[QAT] Starting QAT fine-tuning of FULL generator")
        net_g = prepare_model_for_qat(net_g, backend=qat_backend)
        for param_group in optim_g.param_groups:
            param_group["lr"] = hps.train.learning_rate * 0.1

        hps.train.fp16_run = False
        print("[QAT] fp16_run disabled for QAT")
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d, net_dur_disc, wl, net_wd], [optim_g, optim_d, optim_dur_disc, optim_wd],
                               [scheduler_g, scheduler_d, scheduler_dur_disc, scheduler_wd], scaler, [train_loader, eval_loader],
                               logger, [writer, writer_eval])
        else:
            train_and_evaluate(rank, epoch, hps, [net_g, net_d, net_dur_disc, wl, net_wd], [optim_g, optim_d, optim_dur_disc, optim_wd],
                               [scheduler_g, scheduler_d, scheduler_dur_disc, scheduler_wd], scaler, [train_loader, None], None, None)
        scheduler_g.step()
        scheduler_d.step()
        if net_dur_disc is not None:
    ...
    # ВСТАВКА
    # if getattr(hps.train, "use_qat", False) and rank == 0:
    #     print("[QAT] Converting decoder to int8 and saving quantized checkpoint")

    #     # 1. Восстанавливаем posterior_channels так же, как в run()
    #     if getattr(hps.data, "use_mel_posterior_encoder", False):
    #         posterior_channels = 80
    #     else:
    #         posterior_channels = hps.data.filter_length // 2 + 1

    #     # 2. MAS параметры — тоже как в run()
    #     if getattr(hps.model, "use_noise_scaled_mas", False):
    #         mas_noise_scale_initial = 0.01
    #         noise_scale_delta = 2e-6
    #     else:
    #         mas_noise_scale_initial = 0.0
    ...
    #     # 4. Готовим её к QAT ТЕМ ЖЕ способом, что и net_g в run()
    #     qat_backend = getattr(hps.train, "qat_backend", "fbgemm")
    #     net_g_q = prepare_model_for_qat(net_g_q, backend=qat_backend)

    #     # 5. Теперь архитектура net_g_q и net_g совпадает (оба: без weight_norm и с QAT-обсерверами)
    #     net_g_q.load_state_dict(net_g.state_dict())
    #     net_g_q.eval()

    #     net_g_q = tq.convert(net_g_q, inplace=False)

    #     # 7. Сохраняем quantized-чекпоинт
    #     qat_ckpt_path = os.path.join(hps.model_dir, f"G_QAT_{global_step}.pth")
    #     torch.save(
    #         {
    #             "model": net_g_q.state_dict(),
    #             "iteration": global_step,
    #             "learning_rate": optim_g.param_groups[0]["lr"],
    #         },
    #         qat_ckpt_path,
    #     )
    #     print(f"[QAT] Saved quantized generator to {qat_ckpt_path}")
    if getattr(hps.train, "use_qat", False) and rank == 0:
        print("[QAT] Converting FULL generator to int8 and saving quantized checkpoint")

        # 1. Копируем QAT-модель на CPU
        net_g_cpu = copy.deepcopy(net_g).cpu()
        net_g_cpu.eval()

        # 2. Конвертим в quantized
        net_g_q = tq.convert(net_g_cpu, inplace=False)
        ...
        print(f"[QAT] Saved quantized FULL generator to {qat_ckpt_path}")
...
#        scaler.unscale_(optim_d)
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                ...
                #     # 6. Конвертим только декодер в int8
                #     net_g_q = tq.convert(net_g_q, inplace=False)
