...
### 1) Сначала сделать всё из README по файнтюну — до запуска самого файнтюна
...
- Конфиги:
  - базовый файнтюн без QAT: `db-finetune/config_noQAT.json`
  - остальные параметры модели: `pretrained/config.json`
- Чекпоинты:
  - файнтюн без QAT: `db-finetune/out_ft_XXX/*.pth` (после переименования)
